...
void list_graph::get_table_cornersfxy(Eigen::Vector3i &ixy_r, Eigen::Vector2f *fcorners)
{
    /*
    fcorners[2]  fcorners[1] ^ x
    ...
    */
    ...
    dx = ixy_r(0) * param_interval_x;
    dy = ixy_r(1) * param_interval_y;

    fcorners[0](0) = dx,
     fcorners[0](1) = dy;

    fcorners[1](0) = dx + param_interval_x,
     fcorners[1](1) = dy;

    fcorners[2](0) = dx + param_interval_x,
     fcorners[2](1) = dy + param_interval_y;

    fcorners[3](0) = dx,
     fcorners[3](1) = dy + param_interval_y;
}
                            ...
                            float sx_interval, float sy_interval, float syaw_interval)
{
    ...
    if( (hasinit == false) && srv_map->Map_alloc_sta() && lsr_filter->isfull())
    {
        ...
        fill_node.add_lsrdata(srv_map,
                              Imat.tmatrix, lsr_filter);
        ...
        ROS_INFO(" GRAPH INIT SUCCESS. L %d, W %d", graph_table.length, graph_table.width );
...
{
    const int publish_T = 10;
    static int publish_t = 0; 
