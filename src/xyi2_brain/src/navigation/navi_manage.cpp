...
void navi_manage::globalpath_show(void)
{
    ...
    static ros::Publisher datapub_origin = nstru.advertise<geometry_msgs::PolygonStamped>("path_origin_show",1);
    ...
    if(flag_gpath_show)
    {
        ...
        gpath_show_origin.header.stamp = ros::Time::now();
        gpath_show_origin.header.frame_id = "/map_r";

        datapub.publish(gpath_show);
        datapub_origin.publish(gpath_show_origin);
    }
}
...
void navi_manage::diff_globalpath(void)
{
    ...
    for(j=0; j<_size; j++)
        globalpath_data_origin.push_back(globalpath_data[j]);   
    ...
    gpath_show_origin.polygon.points.clear();
    ...
    for(j=0; j<globalpath_data_origin.size(); j++)
    {
        point_get.x = globalpath_data_origin[j](0);
        point_get.y = globalpath_data_origin[j](1);
        point_get.z = 0;
        
        gpath_show_origin.polygon.points.push_back(point_get);
    }
    for(j=globalpath_data_origin.size()-1; j>=0; j--)
    {
        point_get.x = globalpath_data_origin[j](0);
        point_get.y = globalpath_data_origin[j](1);
        point_get.z = 0;
...
// Func: according to the focus_goal, control the robot based on the local navigation method.
