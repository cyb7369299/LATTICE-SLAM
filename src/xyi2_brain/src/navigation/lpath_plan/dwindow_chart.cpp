                            ...
                            Eigen::Vector2f _focus_goal, Eigen::Vector2f &_vgyro_get)
{
    ...
    k_values = Eigen::Vector4f(0.1f, 0.05f, 0.75f, 0.01f);  // 0.1 0.05 0.75 0.1

    static int i;
    static int _size;
    static Eigen::Vector2f best_v_gyro;
    static float fbest_v[3] = {0, 0, 0};
    ...
    if(_size > 0)
    {
        ...
        for(i=0; i<_size; i++)
        {
            ...
            if(fvalue < fvalues_c_min)
            {
                if(1)
                {
                    fbest_v[0] = k_values(1) * cord_values[i](1) * values_norm(1);
                    fbest_v[1] = k_values(2) * cord_values[i](2) * values_norm(2);
                    fbest_v[2] = k_values(3) * cord_values[i](3) * values_norm(3);
                }
    ...
    if(has_selected_path == false)
    {
        ...
        cout << "[" << fbest_v[0] << ", " << fbest_v[1] << ", " << fbest_v[2] << "],"; 
