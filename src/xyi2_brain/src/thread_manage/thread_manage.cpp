...
void Debug_Matrix4dGetGeoPose(Matrix4d &MatrixDta, geometry_msgs::Pose &posedta);
...
static Eigen::Vector2f sub_odom_xy;
void Thread_Manage::task_sub_odom(const nav_msgs::Odometry& odomdta)
{
    ...
    {
        sub_odom_xy(0) = odomdta.pose.pose.position.x, 
         sub_odom_xy(1) = odomdta.pose.pose.position.y;
        ...
        if(Manage_PramInit)
        {
            if(Enable_Matching)
            {
                ...
                if(MapPubT >= 10)
                {
                    ...
                    #define deal_mat    srv_packdata.Msta_source2robot
                    if(0)
                    {
                        cout << "[" //<< sub_odom_xy(0) << ", "
                                    //<< sub_odom_xy(1) << ", "
                                    << deal_mat(12) << ", "
                                    << deal_mat(13)
                                    //<< std::atan2(deal_mat(1),deal_mat(0))
                                << "], ";
                    }

                    if((NAVI_STA_GPATHPLAN_BUSY == false) && Once_CalLocation && 1)
                    {   // std::atan2(In_MatrixDta.data()[1],In_MatrixDta.data()[0]);
                        // pose_d.position;
                        // pose_d.orientation.x;
                        geometry_msgs::PoseStamped pose_d;
                        pose_d.header.frame_id = "/map_r";
                        pose_d.header.stamp = ros::Time::now();
                        Debug_Matrix4dGetGeoPose(deal_mat, pose_d.pose);
                        
                        path_show.header.stamp = ros::Time::now();
                        path_show.poses.push_back(pose_d);

                        static ros::NodeHandle nstru;
                        static ros::Publisher datapub = nstru.advertise<nav_msgs::Path>("show_path",1);
                        datapub.publish(path_show);
                    }
...
void Debug_Matrix4dGetGeoPose(Matrix4d &MatrixDta, geometry_msgs::Pose &posedta)
{
    ...
    double p; p = PRY[0]*0.5;
    double r; r = PRY[1]*0.5;
    double y; y = PRY[2]*0.5;
